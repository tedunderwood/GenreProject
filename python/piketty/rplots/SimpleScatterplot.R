...
q <- ggplot(filtered, aes(x = date, y = ratio * 10000)) + geom_point() + 
  scale_y_continuous('references to money / 10000 words\n') + scale_x_continuous('') + theme(text = element_text(size = 20)) +
  ggtitle('Number of references to money in fiction\nper ten thousand words, raw yearly count\n') +
