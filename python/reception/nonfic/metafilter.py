...
def balance_classes(metadict, category2sorton, positive_class, sizecap):
    ''' This takes as input the metadata dictionary generated
    by get_metadata. It subsets that dictionary into a
    positive class and a negative class. Instances that belong
    to neither class get ignored.
    ...
    positives = list()
    negatives = list()
    for floor in range(1820, 1920, 10):
        posct = 0
        negct = 0

        for anid in all_instances:
            date = int(metadict[anid]['pubdate'])
            if date >= floor and date < (floor + 10):
                if anid in all_positives and posct < 15:
                    posct += 1
                    positives.append(anid)
                elif anid in all_negatives and negct < 15:
                    negct += 1
                    negatives.append(anid)
    ...
    for key, value in metadict.items():
        if key not in IDsToUse:
            IDsToUse.add(key)
            classdictionary[key] = 2
    # We add the canon supplement, but don't train on it.
