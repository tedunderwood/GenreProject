...
for (center in seq(1750, 1950)) {
  ceiling = center + 12
  floor = center - 12
  moneyvector = money$logval[money$date >= floor & money$date < ceiling]
...
#plot(seq(1750,1950), themedians, type='l', lwd=2, ylim = c(min(themedians), max(themedians)), xlab = '', ylab = 'nominal value in £ \n', main = 'Running median of nominal value, 25 yr window.')
#par(new = T)
df <- data.frame(x = seq(1750,1950), y = themedians)
points = data.frame(x = c(1818,1840,1871,1876, 1925), y = c(1.58, -1.42, -2.12, -0.42, -0.63))
q <- ggplot(df, aes(x=x, y=y)) +  geom_line(colour='cornflowerblue', size = 2) + scale_y_continuous("log10 (nominal value / wage)\n", limits = c(-2.5, 2)) + geom_point(data = points, size = 3)
q <- q + theme(text = element_text(size = 20)) + scale_x_continuous('') + ggtitle('Running 25-year median of \n log (nominal value / wage)\n') + theme(plot.title = element_text(size = 20, lineheight = 1.2))
plot(q)
