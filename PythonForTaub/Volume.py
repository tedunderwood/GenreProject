...
def as_stream(linelist, verbose = False):
    ...
    for index in range(0, listlength):
        ...
        precedingVerse = 0
        for preceder in range(index, index - 6, -1):
            if preceder < 0:
                break
        ...
        followingVerse = 0
        for follower in range(index, index + 3):
        ...
        if precedingVerse + followingVerse > 3 and followingVerse > 0:
            verselist[index] = "-2"
