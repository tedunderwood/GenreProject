...
for (center in seq(1750, 1950)) {
  ceiling = center + 12
  floor = center - 12
  moneyvector = money$nominalval[money$date >= floor & money$date < ceiling]
...
themedians[1:12] <- themedians[13]
themedians[189:201] <-themedians[188]
themeans[1:12] <- themeans[13]
themeans[189:201] <-themeans[188]
...
dm <- data.frame(x = seq(1750,1950), y = themedians)
q <- ggplot(dm, aes(x=x, y=y)) +  geom_area(aes(y=y), fill='cornflowerblue') + scale_y_continuous("value in £")
q <- q + theme(text = element_text(size = 20)) + scale_x_continuous('') + ggtitle('Running 25-year median of \n references to money, nominal value\n') + theme(plot.title = element_text(size = 20, lineheight = 1.2))
plot(q)
